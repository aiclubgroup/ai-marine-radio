...
def transcribe_audio(mic: "MicCapture") -> str:
    """[AI 연동 지점 — STT] 실제 구현: PTT를 누르고 있는 동안 캡처된 오디오
    버퍼(mic)를 Whisper STT 모델에 통과시켜 인식된 한국어 텍스트를 반환한다.
    지금은 데모용 문구뱅크에서 하나를 무작위로 골라 반환하는 시뮬레이션이다."""
...
class MainWindow(QWidget):
    ...
    def _end_ptt(self):
        ...
        # [AI] STT: PTT 동안 캡처된 오디오 -> 한국어 텍스트
        text_ko = transcribe_audio(self.mic)
        # [AI] 번역: 한국어 -> 설정(Settings)에서 고른 번역 언어
